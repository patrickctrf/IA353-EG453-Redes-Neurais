...
def threadDUEL(name):
    ...
    # slows down training quite a lot. You can always safely abort the training prematurely using
    # Ctrl + C.
    duel.fit(env, nb_steps=50000, visualize=False, verbose=2)
...
t1 = threading.Thread(target=threadDQN)
t2 = threading.Thread(target=threadSARSA)
t3 = threading.Thread(target=threadDUEL)
t4 = threading.Thread(target=threadCEM)
...
# Finally, evaluate our algorithm for 5 episodes.
duel.test(env, nb_episodes=5, visualize=False)
